...
### 🔔 价格预警（智能防刷屏）
- 在行情卡片点击「设置预警」→ 对话引导输入阈值，无需记忆指令
- 支持四种条件：**涨幅预警 / 跌幅预警 / 价格上限 / 价格下限**
- **触发后暂停**：条件满足后推送一次，等价格回归正常区间才重新触发，不反复刷屏
- 推送卡片含「✅ 知道了」按钮，点击手动标记已处理
- 仅在 A 股交易时段运行（工作日 9:25–11:35 / 12:55–15:05）
- 支持删除单条预警；检查间隔可在设置里调整（1–60 分钟，建议 ≥5 分钟）
